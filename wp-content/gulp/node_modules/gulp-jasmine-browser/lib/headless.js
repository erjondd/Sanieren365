...
function onError(message) {
  try {
    var _require = require('plugin-error'),
...
function findPort() {
  return _portastic2.default.find({ min: 8000, max: DEFAULT_JASMINE_PORT, retrieve: 1 }).then(function (_ref) {
    var port = _ref[0];
    return port;
  });
}
...
function startServer(files, options) {
  ...
  if (!port) return findPort().then(function (port) {
...
function findOrStartServer(options) {
  function helper(port, streamPort, files) {
    var serverPromise = void 0,
        streamPortPromise = void 0;
    if (!port) serverPromise = startServer(files, options);else serverPromise = _portastic2.default.test(port).then(function (isOpen) {
      if (!isOpen) return { server: { close: function close() {} }, port: port };
      return startServer(files, options);
    });
    if (!streamPort) streamPortPromise = findPort();else streamPortPromise = _promise2.default.resolve(streamPort);
    return _promise2.default.all([serverPromise, streamPortPromise]);
  }

  return (0, _through.obj)(function (files, enc, next) {
    return helper(options.port, options.streamPort, files).then(function (i) {
      return next(null, i);
    }).catch(next);
  });
}
...
function createServer(options) {
  var _options$driver = options.driver,
      driver = _options$driver === undefined ? 'chrome' : _options$driver,
      file = options.file,
    ...
    return (_console = console).log.apply(_console, arguments);
      ...
      withSandbox = options.withSandbox,
      opts = (0, _objectWithoutProperties3.default)(options, ['driver', 'file', 'random', 'throwFailures', 'spec', 'seed', 'reporter', 'profile', 'onCoverage', 'onSnapshot', 'onConsoleMessage', 'withSandbox']);
  ...
  return (0, _multipipe2.default)((0, _through2Reduce.obj)(function (memo, file) {
    return memo[file.relative] = file.contents, memo;
  }, {}), findOrStartServer(options), (0, _flatMap2.default)(function (_ref2, next) {
    var _ref2$ = _ref2[0],
        server = _ref2$.server,
        port = _ref2$.port,
        streamPort = _ref2[1];
    ...
    var phantomProcess = (0, _child_process.spawn)(command, (0, _helper.compact)([runner, port, query]), { cwd: (0, _path.resolve)(__dirname, './runners'), env: env, stdio: stdio });
  ...
  }), (0, _toReporter2.default)(reporter || defaultReporters(opts, profile), { onError: onError, onConsoleMessage: onConsoleMessage, onCoverage: onCoverage, onSnapshot: onSnapshot }));
}
